...
const handleAddOperatorCommand = async (bot, msg) => {
  try {
    ...
    // Phân tích tin nhắn bằng cách tìm các pattern khác nhau
    let username = '';
    
    if (messageText.includes('设置操作人')) {
      const cmdIndex = messageText.indexOf('设置操作人');
      if (cmdIndex !== -1) {
        username = messageText.substring(cmdIndex + 5).trim();
      }
    } else if (messageText.includes('加操作人')) {
      // Hỗ trợ cho cả cú pháp cũ
      const cmdIndex = messageText.indexOf('加操作人');
      if (cmdIndex !== -1) {
        username = messageText.substring(cmdIndex + 4).trim();
      }
    }
